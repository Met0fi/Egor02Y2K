(function () {
  ...
  var LEDGER_KEY = "egor02_ledger_pages";
  ...
  function rotText(root, rift) {
    ...
    while ((node = walk.nextNode())) {
      ...
      if (rift >= 5) t = t.replace(/2004/g, "17?5");
      if (rift >= 6) t = t.replace(/егор/gi, "ег\u00a0р");
      if (rift >= 7) t = t.replace(/вампир/gi, "вам\u00a0пир");
      node.nodeValue = t;
    }
  }
  ...
  if (rift >= 5) {
    ...
    if (mq) mq.textContent = "WELCOME TO The Egor02 site!   ***   файл не совпадает с листом   ***   ";
  }
  if (rift >= 7) {
    document.title = " ";
  }
  ...
  if (listing) {
    ...
    if (rift >= 1) rows.push("guest/befana.png    22-Sep-2026   ??");
      if (rift >= 4) rows.push("kitchen.htm         04-Aug-2004  ????");
    if (rift >= 2) rows.push("fangs.htm           04-Aug-2004  ?");
    if (rift >= 3) rows.push("memories.htm        ????         ");
      if (rift >= 6) rows.push("ledger.htm           ????         read");
      if (rift >= 7) rows.push("cake.htm            ????         forbidden");
  ...
  document.querySelectorAll("[data-ring]").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      var ring = ["guestbook.html", "ledger.html", "bats.html", "old.html"];
      if (dir === "random") {
  ...
  if (ledgerForm) {
    var ledgerPages = [];
    try {
      var storedPages = JSON.parse(localStorage.getItem(LEDGER_KEY) || "[]");
      ledgerPages = Array.isArray(storedPages) ? storedPages.filter(function (page) { return Number.isFinite(Number(page)); }) : [];
    } catch (error) {
      ledgerPages = [];
    }
    var pageCount = ledgerPages.length;
    var pageButtons = document.querySelectorAll("[data-ledger-page]");
    pageButtons.forEach(function (button) {
      button.addEventListener("click", function () {
        var page = Number(button.getAttribute("data-ledger-page"));
        if (!Number.isFinite(page)) return;
        if (ledgerPages.indexOf(page) === -1) ledgerPages.push(page);
        localStorage.setItem(LEDGER_KEY, JSON.stringify(ledgerPages));
        pageCount = ledgerPages.length;
        button.classList.add("read");
        var status = document.getElementById("ledgerStatus");
        if (status) status.textContent = "лист " + pageCount + " из 4 просмотрен";
        if (pageCount >= 4) {
          ledgerForm.hidden = false;
          var lock = document.getElementById("ledgerLock");
    ...
    ledgerForm.addEventListener("submit", function (e) {
      e.preventDefault();
      var form = ledgerForm.elements;
      var answers = [norm(form.year.value), norm(form.name.value), norm(form.word.value), norm(form.date.value)];
      var out = document.getElementById("ledgerFlash");
      if (answers[0] === "1725" && (answers[1] === "frombald" || answers[1] === "фромбальд") && answers[2] === "уголь" && (answers[3] === "0408" || answers[3] === "04.08")) {
        keys = gainKey("ledger");
        out.textContent = "лист закрыт. cvs снова отвечает.";
        var next = document.getElementById("ledgerNext");
        if (next) next.hidden = false;
      } else {
        out.textContent = "поля не сходятся. смотри листы, не угадывай.";
      }
    });
  }
  ...
  if (location.pathname.indexOf("bats") !== -1) {
    var missing = document.querySelector("#batKey");
    if (!missing) missing = document.querySelector("img[src=\"missing-bat.gif\"]");
  ...
  if (location.pathname.indexOf("commits") !== -1) {
    ...
    var commitGate = document.getElementById("commitGate");
    if (keys.indexOf("ledger") === -1) {
      if (commitForm) commitForm.hidden = true;
      if (commitGate) commitGate.hidden = false;
    } else if (commitGate) {
      commitGate.hidden = true;
    }
    if (commitForm) {
